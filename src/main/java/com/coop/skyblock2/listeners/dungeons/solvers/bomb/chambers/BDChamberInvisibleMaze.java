...
public class BDChamberInvisibleMaze extends BDChamber {

	private List<int[]> locations = new ArrayList<>();
	private boolean logicComplete = false;
	
	public BDChamberInvisibleMaze(int[] xzl, int[] xzr, int direction) {
	...
	public void executeLogic(RenderWorldLastEvent e) {
		...
		for (int tx = 0; tx < 9; tx++) {
			for (int tz = 0; tz < 6; tz++) {
				...
				if (b != Blocks.STAINED_HARDENED_CLAY) continue;
